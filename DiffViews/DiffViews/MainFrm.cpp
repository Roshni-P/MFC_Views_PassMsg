...
BEGIN_MESSAGE_MAP(CMainFrame, CMDIFrameWndEx)
	...
	ON_WM_SETTINGCHANGE()
	ON_MESSAGE(ID_PROCESS_VIEW, ProcessView)
END_MESSAGE_MAP()
...
LRESULT CMainFrame::ProcessView(WPARAM wparam, LPARAM lparam)
{
			...
			while (posDoc)
			{
				...
				if (pDoc)
				{
					...
					while (posView)
					{
						...
						if (!str->IsEmpty() && (str->Compare(_T("Even"))==0))
						{
							if (pView->IsKindOf(RUNTIME_CLASS(CEvenView)))
							{
								CEvenView* pEven = (CEvenView*)pView;
								pEven->PostMessageW(ID_PROCESS_EVEN);
							}
						}
						else if (!str->IsEmpty() && (str->Compare(_T("Odd"))==0))
						{
							if (pView->IsKindOf(RUNTIME_CLASS(COddView)))
							{
								COddView* pOdd = (COddView*)pView;
								pOdd->PostMessageW(ID_PROCESS_ODD);
							}
						}
					}
				}
